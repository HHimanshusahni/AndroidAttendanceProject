...
class AddSubject : AppCompatActivity() {
    ...
    lateinit var subjectScheduleList:ArrayList<SubjectSchedule>
    ...
    override fun onBackPressed() {
        ...
        AlertDialog.Builder(this)
                .setMessage("Subject will not be ${temp}\nDo you want to cancel? ")
                .setNegativeButton("NO",{dialog, which ->

                })
                .setPositiveButton("YES",{ dialog, which ->
                    for (schedule in subjectScheduleList)
                        SubjectScheduleTable.addSchedule(db,schedule)
    ...
    fun submitSubject(subjectid: Int?){
        ...
        if(etSubjectName.text.toString().length==0||
        ...
        }else if(etYear.text.toString().toInt()>12){
            Toast.makeText(this,"Year should be less than 13",Toast.LENGTH_SHORT).show()
        }
        else if(etSubjectName.text.toString().length>=15){
            Toast.makeText(this,"Subject Name too big(15 chars allowed)!!!",Toast.LENGTH_SHORT).show()
        }else if(etDepartment.text.toString().length>=10){
            Toast.makeText(this,"Department Name too big(10chars allowed!!!",Toast.LENGTH_SHORT).show()
        }
            ...
            AlertDialog.Builder(this)
                    .setMessage("Do you want to submit the subject ")
    ...
    fun editSubject(subjectId:Int){
        ...
        etYear.setText(""+subject.year)
         ...
         subjectScheduleList =  SubjectScheduleTable.getScheduleFromSubjectId(db,subjectId)
